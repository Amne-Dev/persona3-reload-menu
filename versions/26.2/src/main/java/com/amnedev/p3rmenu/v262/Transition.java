...
public final class Transition {
    ...
    public static boolean blocksScreenInput() {
        return phase != Phase.NONE && Minecraft.getInstance().gui.screen() != null;
    }
    ...
    private static void drawLabel(GuiGraphicsExtractor graphics, int x, int width, int height, float progress) {
        if (label.isBlank()) {
            return;
        }
        ...
        graphics.text(Minecraft.getInstance().font, fragment, 1, 1,
                (Math.min(150, alpha) << 24) | 0x02154E, false);
        graphics.text(Minecraft.getInstance().font, fragment, 0, 0,
